...
//!
//! 2048 English words used for Ajna English Mnemonic (AEM) encoding.
...
/// Find word index using binary search (O(log n))
...
mod tests {
    ...
    fn test_word_to_index() {
        ...
        assert_eq!(word_to_index("invalid_word_xyz"), None);
    }
    ...
    fn test_index_to_word() {
        ...
        assert_eq!(index_to_word(1), Some("ability"));
    ...
    fn test_roundtrip() {
        for i in 0..2048u16 {
            let word = index_to_word(i).unwrap();
            let index = word_to_index(word).unwrap();
            assert_eq!(i, index, "Roundtrip failed for word: {}", word);
        }
    }
}
