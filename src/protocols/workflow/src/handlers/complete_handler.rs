...
use crate::messages::CompleteMessage;
...
impl MessageHandler for CompleteHandler {
    fn supported_types(&self) -> Vec<String> {
        vec![CompleteMessage::TYPE.to_string()]
    }
    ...
    async fn handle(
        ...
        let complete_msg: CompleteMessage = serde_json::from_value(inbound.message.body.clone())
            .map_err(|e| MessageHandlerError::InvalidMessage(e.to_string()))?;

        self.command_queue
            .enqueue(
                CommandType::Complete,
                &complete_msg.instance_id,
                inbound.context.connection_id.as_deref(),
                None,
                serde_json::to_value(&complete_msg)
