...
impl DidResolver for PeerDidResolver {
    ...
    async fn resolve(&self, did: &DID) -> ResolutionResult<DidDocument> {
        // Check if this is a did:peer:1 DID (starts with "did:peer:1")
        // Resolves did:peer:1 by looking up the stored genesis document
        if did.as_str().starts_with("did:peer:1") {
            // Query primary DidRepository for stored genesis document
            if let Some(did_record) = self.did_repository.find_by_did(did.as_str()) {
                if let Some(doc) = did_record.did_document {
                    return Ok(doc);
                }
                return Err(ResolutionError::InternalError(format!(
                    "DidRecord found for {} but didDocument is missing",
                    did.as_str()
                )));
            }
            // Fall back to the shared/root repo if one was attached.
            // This is what lets tenant resolvers find the mediator's DID
            // doc, which is stored only on `SharedInfrastructure` at
            // mediation-grant time.
            if let Some(fallback) = &self.fallback_repository {
                if let Some(did_record) = fallback.find_by_did(did.as_str()) {
                    if let Some(doc) = did_record.did_document {
                        return Ok(doc);
                    }
                }
            ...
            return Err(ResolutionError::NotFound(
                format!("No did record found for peer did {} - did:peer:1 resolution requires the genesis document", did.as_str())
            ));
        }

        // For did:peer:2 and other numalgo, use the did-peer crate
        let result = self.peer.resolve(did.as_str()).await.map_err(|e| {
            ResolutionError::ResolutionFailed(format!("did:peer resolution failed: {:?}", e))
        })?;

        // The result IS the Document, not wrapped
